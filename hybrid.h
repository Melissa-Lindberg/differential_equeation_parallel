...
{
    if (M == 10 && N == 10) return {2, 2};
    if (M == 20 && N == 20) return {4, 4};
    if (M == 40 && N == 40) return {8, 8};
    if (M == 400 && N == 600) return {8, 12};
    if (M == 800 && N == 1200) return {16, 24};
    return {0, 0};
}
...
struct HaloBuffers {
    std::vector<double> send_west;
    std::vector<double> recv_west;
    std::vector<double> send_east;
    std::vector<double> recv_east;
    std::vector<double> send_south;
    std::vector<double> recv_south;
    std::vector<double> send_north;
    std::vector<double> recv_north;
};
